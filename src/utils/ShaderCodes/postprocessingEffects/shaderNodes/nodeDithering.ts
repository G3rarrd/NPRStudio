...
import ShaderDithering from "../nonCompositeTextures/shaderDithering";
...
const SLIDER_CONFIG = [
    {label : "spread value", min: 0, max: 10, step: 0.01, value: 0.3 },
    {label : "bayer type", min: 1, max: 4, step: 1, value: 1 },
]

export class NodeDithering implements ShaderNode {
    public readonly  filter : ShaderDithering;
    ...
    constructor (id : number, pool : FramebufferPool, wgl : WebGLCore) {
        ...
        this.filter = new ShaderDithering(wgl);
   ...
   public buildSliders () : void {
    ...
    public updateUniformValues () : void {
        const spreadValue : number = this.sliderMap["spread value"].value;
        const bayerType : number = this.sliderMap["bayer type"].value;

        this.filter.setUniformValues(bayerType, spreadValue);
    }
