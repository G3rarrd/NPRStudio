...
const SLIDER_CONFIG = [
    {label : "threshold", min: 0, max: 1, step: 0.01, value: 0.3 },
]

export class NodeBinaryThreshold implements ShaderNode {
    public readonly  filter : ShaderBinaryThreshold;
    ...
    constructor (
        ...
        this.pool = pool;
        // this.wgl = wgl;
        this.filter = new ShaderBinaryThreshold(wgl);
    ...
    public updateUniformValues () : void {
        const threshold : number = this.sliderMap["threshold"].value;

        this.filter.setUniformValues(threshold);
    }
...
export default NodeBinaryThreshold;
