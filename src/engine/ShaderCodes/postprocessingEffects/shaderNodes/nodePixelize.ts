...
import ShaderPixelize from "../nonCompositeTextures/shaderPixelize";
...
const SLIDER_CONFIGS = [{label : "block size", min:1, max: 100, step: 1, value: 5 }];

export class NodePixelize implements ShaderNode {
    public readonly  filter : ShaderPixelize;
    ...
    constructor (
        id : number, pool : FramebufferPool, wgl : WebGLCore) {
        this.id = id;
        this.pool = pool;

        this.filter = new ShaderPixelize(wgl);
    ...
    public updateUniformValues () : void {
        const colorCount : number = this.sliderMap["block size"].value;
